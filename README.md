...
###### 	03  querystring模块   ==处理的qurey类型参数==

**03.1 parse**  ==第二个参数为true，解析出来的query为一个对象（默认false，解析出来为正常query参数格式）==
...
**03.3 escape/unescape**  ==转义字符（防止sql等注入字符影响系统）==
...
const app = http.createServer((req, res) => {
  let urlObj = url.parse(req.url, true) //第二个参数为true，解析出来的query为一个对象（默认false，解析出来为正常query参数格式）
...
const server = http.createServer((request, response) => {
  ...
  https.get(`https://m.lagou.com/listmore.json${url}`, (res) => {
    ...
    //res.on ‘data’ 监听（一段一段chunk拼接接收）
    res.on('data', (chunk) => {
      data += chunk
    })
		//res.on监听需要配合结束字符‘end’
    res.on('end', () => {
...
**04.4 模拟post：服务器提交（攻击）**  ==post数据放在请求体中==
...
让我们用`pipe()`把一个文件流和另一个文件流串起来，这样源文件的所有数据就自动写入到目标文件里了，所以，这实际上是一个==复制文件==的程序：
...
1. link href="/css/xxx.css"

2. 这样引入文件肯定要”报错“，走404route，404路由中封装相应的静态资源处理函数
...
function readStaticFile(req, res) {
    ...
    //绝对路径访问  path模块处理，不同系统下'/\' 不同的情况
    var filePathname = path.join(__dirname, "/static", myURL.pathname);

    if (fs.existsSync(filePathname)) {
        // console.log(1111)
      	//mime模块                                           //获取pathname点分开，获取生成数组第二个元素，即为文件类型，比如css
...
路由路径和请求方法一起定义了请求的端点，它可以是==字符串、字符串模式或者正则表达式==。
...
可以为<u>请求处理提供多个回调函数</u>，其行为类似 ==中间件==。唯一的区别是这些回调函数有可能调用 next('route') 方法而略过其他路由回调函数。可以利用该机制为路由定义前提条件，如果在现有路径上继续执行没有意义，则可将控制权交给剩下的路径。
...
// 没有挂载路径的中间件(万能匹配)，应用的每个请求都会执行该中间件
...
##### 7.服务端渲染（模板引擎）server side render
...
express [名(myapp)] --view=ejs //默认生成jade模板
进入myapp 目录，npm install
```
...
连接数据库  (config/db.config.js)   <u>再将模块引入至/bin/www</u>
...
//一个模型对应数据库的集合collection

创建模型（model/UserModel）
...
const UserModel = mongoose.model("user",new Schema(UserType))//限制user类型
...
//只输出username,role,introcduction,password字段内容，排序（按照createTime，-1 从大到小） 

//分页 skip((page-1)*limit).limit(limit) 
```
...
controller层：接收（接受来自model层的数据），返回（返回给view层）数据
...
「HTTP 无状态」**我们知道，<font color=red>HTTP 是无状态的</font>。也就是说，HTTP 请求方和响应方间无法维护状态，都是一次性的，它不知道前后的请求都发生了什么。但有的场景下，我们需要维护状态。最典型的，一个用户登陆微博，发布、关注、评论，都应是在登录后的用户状态下的。**「标记」那解决办法是什么呢？![image-20220414095345868](%E7%AC%94%E8%AE%B0.assets/image-20220414095345868.png)



cookie（存于客户端）  容易被伪造，不用给后端校验

配合session ，session存在后端   可以在**后端销毁**（session基于cookie）  （session存于服务端（内存/数据库）

每一个cookie存入相应sessionid（钥匙），用于打开session库 （房间门）<font color=red>？</font>跳转其他页面<font color=red>：</font>重定向登录页
...
app.js文件注册模块，并且在**应用路由中间件**之前use这个模块
...
 <u>过期校验session，每次放行通过前记得更新session，以保证日期失效时间随着每次访问变化</u>
...
app.use(
  session({
    secret: "this is session", // 服务器生成 session 的签名  秘钥（防止彩虹表比对）
    resave: true, //重新设置session后会，重新设置过期时间
...
app.use((req,res,next)=>{
  ...
  //在登录路由中，如果session中成功挂载了session的user对象，就重新渲染页面
  if(req.session.user){
...
第一种，机器a存储了session，但在机器b没存储，需要复制给b

第二种，session单独存于一个地方，所有机器直接取。但是session存储炸了，所有机器上的session就无了，要重新登录 

安全性：基于cookie这种方式取的数据 ，浏览器自动将cookie带给后端，后端自己会校验。



Session -- 每个请求都要做session校验（所以在应用级中间件中设置）

Jwt 在前端设置拦截器，用于在每次请求后设置请求头、LocalStorage 



CSRF(Cross-Site Request Forgery)跨站请求伪造

a.com cookie   b.com(http:a.com?from=1&to=2 cookie)



用localstorage，只有调用才会更改。但是在前端会被篡改 
...
前端发送ajax带上token值，存入localstorage中 
...
setTimeout(() => {
    let decoded = jwt.verify(token, '秘钥')
    console.log(decoded)
}, 11000)
```
...
请求==成功==之后第一个使用方法 	axios().==(在这个阶段调用方法哟)==then(res=>...)
...
**注意**: Multer 不会处理任何==非== `multipart/form-data` 类型的表单数据。

```
npm install --save multer
```

<u>//利用form表单提交的话，记得写enctype="multipart/form-data"</u>
...
> //上传多个文件，前端input 标签 多加一个multiple
>
> 后端：
>
> ```
